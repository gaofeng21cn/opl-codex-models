...
- A：工具协议兼容。只转换选中的准确模型名，未选模型原样通过。当前 DeepSeek Flash 适配支持 `deepseek-flash` 与 `deepseek-v4.1-flash` 两个 ID；如果目录里保留两个条目，桥的作用范围也必须逐个选择实际使用的 ID。
- B：实验性的上下文纠偏；不修改本地保存的历史。
- AB：同时使用。0：两者关闭。
- 压缩请求始终原样透传，压缩后的普通工具请求继续按模式处理。
- GPT 与 DeepSeek 共用同一个 provider 时，即使未选模型也依赖桥运行。
- 模型管理页默认显示 Codex 当前实际使用的模型目录（来自连接页选中的 config.toml）。
  常用操作只有“添加模型 / 编辑 / 删除”，改动先落在自动创建的编辑副本里；
  点“查看并应用修改…”才会写回，写入前自动备份，删除需再次确认。
  编辑可改模型 ID（重复会被拒绝）；官方同步、导入导出、备份恢复、运行时验证在“高级操作”里。
  目录条目不代表中转一定支持该模型；“验证兼容性”只证明本机 Codex 能读取隔离的
  model_catalog_json（不发起模型调用），不验证中转能力或图片/推理档位。
  若 config.toml 还没引用模型目录，“查看并应用修改”会先发布一个独立的生效目录文件
  并写好引用（预览显示真实路径，原文件先备份）；撤销用“高级操作 → 撤销首次应用”。

“刷新状态”读取配置及本地健康状态，不发送模型请求。“导出诊断”不含 Key、配置正文、对话或代码。
协议桥的认证仍由 Codex 提供，管理器不会保存或展示 Key。模型目录在线刷新会把所选运行时的 `auth.json` 临时复制到隔离目录；刷新结束即清理，不会复制 `config.toml`。
兼容桥是可关闭的临时方案，不承诺修复所有工具问题。GPT 直连稳定时可以保持桥关闭；需要 DeepSeek Flash 工具协议兼容时，只把实际使用的 DeepSeek ID 加入作用范围。
